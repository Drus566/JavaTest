...
public class TableColumnModelTest extends JFrame
{
    ...
    private Object[][] array = new String[][] {{ "Сахар" , "кг", "1.5", "44" },
            ...
            { "Молоко", "л" , "2.2", "45" }};
    // Заголовки столбцов
    private Object[] columnsHeader = new String[] {"Наименование private ssadad", "Ед.измерения", "Количество"};
    int x = columnsHeader[0].toString().length();
    ...
    public TableColumnModelTest()
    {
        ...
        Font sampleFont = new Font("SansSerif", Font.BOLD, fontSize);
        table1.getTableHeader().setFont(sampleFont);
        Dimension tableHeaderSize = table1.getTableHeader().getPreferredSize();
...
//            column.setMaxWidth(200);
        ...
        FontMetrics fm = img.getGraphics().getFontMetrics(sampleFont);
        int width = fm.stringWidth(columnsHeader[0].toString()) + 10;
...
//                table1.getTableHeader().getColumnModel().getColumn(0).getHeaderValue();
                if (columnModel.getColumn(0).getWidth() < width) {
                    table1.getTableHeader().setPreferredSize(new Dimension(tableHeaderSize.width, tableHeaderSize.height * 2));
                } else {
                    table1.getTableHeader().setPreferredSize(tableHeaderSize);
                }
                System.out.println(width);
